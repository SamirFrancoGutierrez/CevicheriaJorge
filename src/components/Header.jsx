...
export const Header = ({
  ...
  const onDeleteProduct = (product) => {
    ...
    const updatedCountProducts = updatedProducts.reduce(
  ...
  const onClearCart = () => {
    setAllProducts([]); // Eliminar todos los productos del carrito
    setCountProducts(0); // Reiniciar el contador de productos
    setTotal(0); // Reiniciar el total
  };

  return (
    <header>
      <h1>Cevichería Jorgito</h1>
                ...
                {allProducts.map((product) => (
                  <div className="cart-product" key={product.id}>
                    <div className="info-cart-product">
                      ...
                      <span className="precio-producto-carrito">S/{product.price}</span>
                    </div>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
